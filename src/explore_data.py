...
# Process in chunks because the file is large (~1.84M rows)
chunk_size = 200000
total_rows = 0
uk_ire_count = 0

# Final UK + Ireland list
uk_ire_courses = {
...
for chunk in pd.read_csv('database/raceform.csv', chunksize=chunk_size):
    chunk['course'] = chunk['course'].astype(str).str.strip()
    total_rows += len(chunk)
    uk_ire_count += chunk['course'].isin(uk_ire_courses).sum()
...
print("="*50)
print(f"Total rows in raceform.csv     : {total_rows:,}")
print(f"UK + Ireland rows              : {uk_ire_count:,}")
print(f"Percentage UK/Ire             : {round(uk_ire_count/total_rows*100, 2)} %")
